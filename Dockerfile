...
RUN wget https://raw.githubusercontent.com/frappe/bench/master/playbooks/install.py && \
    ...
    python install.py --production --bench-branch master --frappe-branch staging --erpnext-branch staging --user $FRAPPE_USER --mysql-root-password $MYSQL_PASSWORD --admin-password $ADMIN_PASSWORD && \
